...
public class DatabaseQueries {
    ...
    public static void getTotalNumberOfiPhones(Connection conn) throws SQLException {
        String sql = "SELECT COUNT(*) AS total_iphones FROM smartphones WHERE Brand like '*iPhone*' ";
    ...
    public static void getPhonesUnder500(Connection conn) throws SQLException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {
            while (rs.next()) {
                System.out.println("Smartphone: " + rs.getString("Smartphone") + ", Model: " + rs.getString("Model") + ", Price: " + rs.getBigDecimal("Final_Price"));
            }
        }
    }
    ...
    public static void getCheapestNothingPhone(Connection conn) throws SQLException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql);
             ResultSet rs = stmt.executeQuery()) {
            if (rs.next()) {
                System.out.println("Cheapest Nothing phone: " + rs.getString("Smartphone") + ", Model: " + rs.getString("Model") + ", Price: " + rs.getBigDecimal("Final_Price"));
            }
        }
    }
